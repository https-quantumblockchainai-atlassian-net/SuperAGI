...
flp_ask_ws_user(Query,Answer) :-
	...
	atom_concat('jaswipl-call python ',TmpQueryAtom,QueryAtom),
	view([queryAtom,QueryAtom]),
	atomic_list_concat(['unilang-client -q -r FCMS -c "',QueryAtom,'"'],'',Command),
	shell_command_to_string(Command,Result),
	view([result,Result]),
	%% flp_ask_user(Query,Answer),
	getPythonMessage(Answer).

%% getPythonMessage(Answer) :-
%% 	repeat,
%% 	findall(Message,pythonMessage(Message),Messages),
%% 	length(Messages,L),
%% 	(   (	L > 0) ->
%% 	    (	view([messages,Messages]),Messages = [Answer|_],retractall(pythonMessage(_)),!) ;
%% 	    (	sleep(1),
%% 		fail)).
...
getPythonMessage(Answer) :-
	...
	length(Messages,L),
	(   (	L > 0) ->
	    (	view([pythonMessages,Messages]),(Messages = [Answer|_] -> true ; true),retract(pythonMessage(Answer)),!) ;
	    (	sleep(1),
		fail)).

python_respond_to_jason(List,Result) :-
	correctLists(List,[username,UserName,message,Message]),
	assertz(pythonMessage(Message)).
...
python_respond_to_jason_canned(List,Answer) :-
	correctLists(List,[username,UserName,message,Message]),
	view([python_respond_to_jason_canned(Question,Answer)]),
	hasCannedReplyConcat(Message,Answer),
	view([python_respond_to_jason_canned_answer,Answer]),
	assertz(pythonMessage(Answer)).

cleanPythonMessages :-
	retractall(pythonMessage(_)).