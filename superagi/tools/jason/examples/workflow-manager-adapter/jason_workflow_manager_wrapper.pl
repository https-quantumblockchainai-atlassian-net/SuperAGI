...
:- ensure_loaded('/var/lib/myfrdcsa/collaborative/git/jason/examples/workflow-manager-adapter/jason_workflow_manager_helper.pl').

isa(workflowManager,actor).

flag(jasonIntegration,true).
flag(log,10).

%% :- consult('/var/lib/myfrdcsa/codebases/minor/agentspeak-frdcsa/jason/environments/workflow-manager/workflow_manager.pl').

prolog_consult(File) :-
	consult(File).

:- consult('/var/lib/myfrdcsa/codebases/minor/formalog-pengines/formalog_pengines.pl').
:- start_agent(jasonWorkflowManager).
...
%% /var/lib/myfrdcsa/collaborative/git/jason/examples/lworkflow-manager-adapter/src/java/WorkflowManagerAdapterEnv.java
